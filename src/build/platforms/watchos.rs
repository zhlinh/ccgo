...
impl PlatformBuilder for WatchosBuilder {
    ...
    fn build(&self, ctx: &BuildContext) -> Result<BuildResult> {
        ...
        let include_source = if built_link_types.contains(&"static") {
            // Try to find a build directory with include files
            let arch = if ctx.options.architectures.contains(&"arm64_32".to_string()) {
                "arm64_32"
            } else {
                "armv7k"
            };
            ctx.cmake_build_dir.join(format!("static/device/{}/include", arch))
        } else {
            let arch = if ctx.options.architectures.contains(&"arm64_32".to_string()) {
                "arm64_32"
            } else {
            ...
            ctx.cmake_build_dir.join(format!("shared/device/{}/include", arch))
        };
